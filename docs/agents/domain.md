...
suggest creating them upfront. The producer skill (`/grill-with-docs`) creates them lazily when
...
> If the glossary grows enough that distinct sub-domains (e.g. `element`, `math`, the editor
> library, the app/collab layer) each need their own language, switch to a multi-context layout:
> a `CONTEXT-MAP.md` at the root pointing to one `CONTEXT.md` per context. Not needed yet.
